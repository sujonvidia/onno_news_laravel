                                            ...
                                            @if($post->post_type=='video')
                                                @if($post->video_id != null)
                                                    ...
                                                    @if(isFileExist(@$post->videoThumbnail, @$post->videoThumbnail->big_image_two))
                                                                <video  id='my-video' controls poster="{{basePath($post->videoThumbnail)}}/{{ $post->videoThumbnail->big_image_two }} " width="100%" height="100%">
                                                            @else
                                                                <video id='my-video' controls poster="{{static_asset('default-image/default-730x400.png') }}">
                                                            @endif
                                                            ...
                                                            @if($post->video->v_144p==null and
                                                                ...
                                                                <source data-fluid-hd src='{{ basePath($post->video) }}/{{ $post->video->original }}' title='1080p' type='video/{{$post->video->video_type}}' />
                                                            @else
                                                                ...
                                                                @foreach($video_version as $version)
                                                                    @if($post->video->$version !=null)
                                                                        <source @if($version =="v_720p" || $version =="v_1080p")data-fluid-hd @endif src='{{ basePath($post->video) }}/{{ $post->video->$version }}' title='{{ str_replace("v_","",$version) }}'  type='video/{{$post->video->video_type}}' />
                                                                    @endif
                                                                @endforeach
                                                            @endif
                                                        </video>
                                                ...
                                                @else
                                                    @if($post->video_url_type == "youtube_url")
                                                        <div class="embed-responsive embed-responsive-16by9">
                                                            <iframe class="embed-responsive-item" src="https://www.youtube.com/embed/{{get_id_youtube($post->video_url)}}?rel=0" allowfullscreen=""></iframe>
                                                        </div>
                                                    @elseif($post->video_url_type == "mp4_url")
                                                        <video  id='my-video' controls poster="{{ asset(@$post->image->big_image_two) }}" width="100%" height="100%">
...
@section('script')
@if($post->post_type =='video')
<script src="{{static_asset('site/js') }}/fluidplayer.min.js"></script>
<script src="{{static_asset('site/js') }}/fluidplayerCustom.js"></script>
@endif
