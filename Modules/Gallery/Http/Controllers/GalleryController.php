...
class GalleryController extends Controller
{
    ...
    public function deleteImage(Request $request){
        ...
        elseif($image->disk =='local'):
            if ($image->count() > 0) :
                if (File::exists($image->original_image)) :
                    unlink($image->original_image);
                endif;
                if (File::exists($image->og_image)) :
                    unlink($image->og_image);
                endif;
                if (File::exists($image->thumbnail)) :
                    unlink($image->thumbnail);
                endif;
                if (File::exists($image->big_image)) :
                    unlink($image->big_image);
                endif;
                if (File::exists($image->big_image_two)) :
                    unlink($image->big_image_two);
                endif;
                if (File::exists($image->medium_image)) :
                    unlink($image->medium_image);
                endif;
                if (File::exists($image->medium_image_two)) :
                    unlink($image->medium_image_two);
                endif;
                if (File::exists($image->medium_image_three)) :
                    unlink($image->medium_image_three);
                endif;
                if (File::exists($image->small_image)) :
    ...
    public function deleteVideo(Request $request){
        ...
        elseif($video->disk =='local'):
            if ($video->count() > 0) :
                if (File::exists($video->original)) :
                    unlink($video->original);
                endif;
                if (File::exists($video->p_144)) :
                    unlink($video->p_144);
                endif;
                if (File::exists($video->p_240)) :
                    unlink($video->p_240);
                endif;
                if (File::exists($video->p_360)) :
                    unlink($video->p_360);
                endif;
                if (File::exists($video->p_480)) :
                    unlink($video->p_480);
                endif;
                if (File::exists($video->video_thumbnail)) :
                    unlink($video->video_thumbnail);
