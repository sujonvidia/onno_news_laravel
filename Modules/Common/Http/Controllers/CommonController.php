...
class CommonController extends Controller
{
    ...
    public function index()
    {
        ...
        $postTrackers                  = VisitorTracker::with('post.image')->where('page_type', \App\Enums\VisitorPageType::PostDetailPage)->get();

        $postTrackers                  = $postTrackers->groupBy('slug');
        $data['post_detail'] = [];
        foreach($postTrackers as $key => $postTracker){
            $data['post_detail'][]     = array('hits'=>$postTracker->count(), 'post' => $postTracker->first());
        }

        array_multisort( array_column($data['post_detail'], "hits"), SORT_DESC, $data['post_detail'] );

        array_slice($data['post_detail'],0,4);
